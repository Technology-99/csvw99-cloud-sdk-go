...
import (
	...
	"github.com/Technology-99/csvw99-cloud-sdk-go/pb/wechat"
...
func (c *Sdk) WechatMiniAppCode2Session(in *wechat.CodeReq) (*wechat.MiniAppCodeResp, error) {
	// note: 读取当前使用配置
	if c.Wechat.UsingConfig == nil {
		c.Wechat.UsingConfig = c.Wechat.Configs["default"]
	}
	res, err := c.Wechat.MiniAppCode2Session(c.SonyCtx(), in)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func (c *Sdk) WechatOfficialAccountJsApiTicket(in *wechat.OaKeyReq) (*wechat.OaJsApiTicketResp, error) {
	...
	res, err := c.Wechat.OfficialAccountJsApiTicket(c.SonyCtx(), in)
	if err != nil {
		return nil, err
	}
	return res, nil
