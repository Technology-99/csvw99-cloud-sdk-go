...
import (
	...
	"github.com/Technology-99/csvw99-cloud-sdk-go/pb/sms"
...
func (c *Sdk) SmsSendSmsAndKey(key, phone string, args ...any) (*sms.SmsResp, error) {
	// note: 读取当前使用配置
	config := &cloudc.ModelSmsConfig{}
	if _, ok := c.Sms.Configs[key]; !ok {
		logx.Infof("config not exist, use default")
		config = c.Sms.Configs["default"]
	} else {
		config = c.Sms.Configs[key]
	}
	param := make(map[string]string)
	tempStr := fmt.Sprintf(config.ParamsStr, args...)
	if err := json.Unmarshal([]byte(tempStr), &param); err != nil {
		return nil, err
	}
	res, err := c.Sms.SendSms(c.SonyCtx(), &sms.SmsParams{
